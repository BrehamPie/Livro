...
if ($_POST['action'] == 'general') {
    $curFullName = $_POST['full_name'] ?? $userData['fullname'];
    $curPhone = $_POST['phone'] ?? $userData['phone'];
    $curDiv = $_POST['division'] ?? $userData['division'];
    $curDis = $_POST['district'] ?? $userData['district'];
    $curUpa = $_POST['upazila'] ?? $userData['upazilla'];
    $curLoc = $_POST['local_ad'] ?? $userData['local_address'];
    ...
    updateUser($uid, $curFullName, $curPhone, $curDiv, $curDis, $curUpa, $curLoc, $curAbt, $basename);
} else {
